...
using RcDeskPilot.Interfaces;
using System.Threading;

namespace RcDeskPilot
{
    internal class FlightModelWind2 : IAirplaneControl, IDisposable, IFlightModel
    {
        ...
        public void MoveScene(float elapsedTime)
        {
            lock (this)
            {
                ...
                {
                    if (Vx > 5f)
                        AAz += (float)(Rudder * (10f - Vx) - Wz) * (20f / (float)AircraftParameters.Mass);
                    else
        ...
        protected double DragLeft { get; set; }
        protected double DragRight { get; set; }
        protected double Thrust { get; set; }
        protected double SideLift { get; set; }
        ...
        private void CalculateForces(float elapsedTime, Vector3 wind)
        {
            ...
            DragLeft = GetDragForceWing(AlphaLeft, new Vector3(AircraftParameters.WingCenter.X, -AircraftParameters.WingCenter.Y, AircraftParameters.WingCenter.Z), leftWingWind);
...
#endif
            //Framework.Instance.DebugString = string.Format("Yaw: {0}, Pitch: {1}, Roll {2}",
            //    (Yaw * 180.0 / Math.PI).ToString("F00"), (Pitch * 180.0 / Math.PI).ToString("F00"), (Roll * 180.0 / Math.PI).ToString("F00"));
            Lift = LiftLeft + LiftRight;
            Drag = DragLeft + DragRight;
            SideLift = GetSideLiftForce(wind);
            ...
            Thrust = AircraftParameters.MaximumThrust * Throttle * AircraftParameters.GetThrustCoefficient((double)Vx);
            Fx = (float)(+SideLift * Math.Sin(Beta)
                         - sideDrag * Math.Cos(Beta)
                         - LiftLeft * Math.Sin(AlphaLeft) - LiftRight * Math.Sin(AlphaRight)
                         - DragLeft * Math.Cos(AlphaLeft) - DragRight * Math.Cos(AlphaRight)
                         //- Drag * Math.Sign(Vx)
                         + Thrust
            ...
            //Fz = (float)(-Lift * Math.Sign(Vx) + gravityMass * Math.Cos(Roll) * Math.Cos(Pitch));
            Fz = (float)(- LiftLeft * Math.Cos(AlphaLeft) - LiftRight*Math.Cos(AlphaRight)
                         - DragLeft * Math.Sin(AlphaLeft) - DragRight * Math.Sin(AlphaRight)
                         + gravityMass * Math.Cos(Roll) * Math.Cos(Pitch));
        }
        ...
        private void CalculateForcesSailboat(float elapsedTime, Vector3 wind)
        {
            ...
            if (Math.Abs(wind.X) > 0)
                sailAlpha = -Math.Atan2(wind.Y, wind.X);
            sailAlpha += Ailerons;
            sideGroundResistance = 20*Math.Sin(realBeta) * gravityMass;

            float saillift = 150*(float)rhoSurface;
            float totalDrag = Vx * Vx + Vy * Vy;
            if (Vx < 0)
                totalDrag *= 2;
            Fx = (float)(- saillift * Math.Cos(sailAlpha) + 25*(Throttle + 1.0)
                         - totalDrag * Math.Sign(Vx)
                         - gravityMass * Math.Sin(Pitch)
                         - groundResistance);
            Fy = (float)(+saillift * Math.Sin(sailAlpha) + gravityMass * Math.Sin(Roll) * Math.Cos(Pitch) + sideGroundResistance);
        ...
        /// <summary>
        /// Calculates the lift force of one wing.
        /// </summary>
        /// <param name="wingCenter"></param>
        /// <param name="wind"></param>
        /// <returns></returns>
        private double GetLiftForceWing(double wingAlpha, Vector3 wingCenter, Vector3 wind)
        {
            Vector3 wingVelocity = new Vector3(Vx, 0, Vz) + Vector3.Cross(new Vector3(Wx, 0, Wz), wingCenter);
            wingAlpha -= AircraftParameters.AileronEfficiency * Ailerons * Math.Sign(wingCenter.Y);
            if (AircraftParameters.HasFlaps)
            {
                double noFlaps = AircraftParameters.GetLiftCoefficient(wingAlpha);
                double withFlaps = AircraftParameters.GetLiftCoefficientWithFlaps(wingAlpha);
                return (Flaps * withFlaps + (1 - Flaps) * noFlaps) * ((wingVelocity.X + wind.X) * (wingVelocity.X + wind.X) + (wingVelocity.Z + wind.Z) * (wingVelocity.Z + wind.Z)) * rhoSurface / 2.0;
            }
            else
            {
                //return (AircraftParameters.GetLiftCoefficient(wingAlpha) - 10 * AircraftParameters.AileronEfficiency * Ailerons * wingCenter.Y) *
                //    ((wingVelocity.X + wind.X) * (wingVelocity.X + wind.X) + (wingVelocity.Z + wind.Z) * (wingVelocity.Z + wind.Z)) * rhoSurface / 2.0;
                return AircraftParameters.GetLiftCoefficient(wingAlpha) *
                 ((wingVelocity.X + wind.X) * (wingVelocity.X + wind.X) + (wingVelocity.Z + wind.Z) * (wingVelocity.Z + wind.Z)) * rhoSurface / 2.0;
            }
        }
        ...
        /// <summary>
        /// Calculates the drag force of one wing.
        /// </summary>
        /// <param name="wingCenter"></param>
        /// <param name="wind"></param>
        /// <returns></returns>
        private double GetDragForceWing(double wingAlpha, Vector3 wingCenter, Vector3 wind)
        {
            Vector3 wingVelocity = new Vector3(Vx, 0, Vz) + Vector3.Cross(new Vector3(Wx, 0, Wz), wingCenter);
            wingAlpha -= AircraftParameters.AileronEfficiency * Ailerons * Math.Sign(wingCenter.Y);
            if (AircraftParameters.HasFlaps)
            {
                double noFlaps = AircraftParameters.GetDragCoefficient(wingAlpha);
                double withFlaps = AircraftParameters.GetDragCoefficientWithFlaps(wingAlpha);
                return (Flaps * withFlaps + (1 - Flaps) * noFlaps) * ((wingVelocity.X + wind.X) * (wingVelocity.X + wind.X) + (wingVelocity.Z + wind.Z) * (wingVelocity.Z + wind.Z)) * rhoSurface / 2.0;
            }
            else
                return AircraftParameters.GetDragCoefficient(wingAlpha) *  ((wingVelocity.X + wind.X) * (wingVelocity.X + wind.X) + (wingVelocity.Z + wind.Z) * (wingVelocity.Z + wind.Z)) * rhoSurface / 2.0;
        }

        private double GetAlpha(Vector3 wingCenter, Vector3 wind)
        {
            double wingAlpha = 0;
            Vector3 wingVelocity = new Vector3(Vx, Vy, Vz) + Vector3.Cross(new Vector3(Wx, Wy, Wz), wingCenter);
            if (Math.Abs(wingVelocity.X + wind.X) > 0.001)
            {
                wingAlpha = Math.Atan2(wingVelocity.Z + wind.Z, wingVelocity.X + wind.X);
            }
            wingAlpha -= wingCenter.Y * AircraftParameters.DihedralAngle * AircraftParameters.DihedralEfficiency * Math.Sin(Beta);
            return wingAlpha;
        }
        ...
        private void CalculateTorques(float elapsedTime, Vector3 wind)
        {
            ...
            Vector3 gravityPlane = ToPlaneCoords(new Vector3(0, 0, (float)gravityMass));
            double totalLift = Math.Abs(Lift) + Math.Abs(SideLift);
            Tx = (float)(-AircraftParameters.RollDamping * Wx
                          + ((LiftLeft * Math.Cos(AlphaLeft)*Math.Sign(Math.Cos(AlphaLeft)) - LiftRight * Math.Cos(AlphaRight)*Math.Sign(Math.Cos(AlphaRight)))
                          + (DragLeft * Math.Sin(AlphaLeft) - DragRight * Math.Sin(AlphaRight))) 
                                * AircraftParameters.WingSpanFactor * AircraftParameters.WingCenter.Y 
                          //+ (LiftLeft - LiftRight) * AircraftParameters.WingSpanFactor * AircraftParameters.WingCenter.Y 
                          + AircraftParameters.AileronEfficiency * AircraftParameters.PropWashAilerons * Throttle * Ailerons
                          + gravityPlane.Y * AircraftParameters.WingCenter.Z);
            Ty = (float)( -AircraftParameters.PitchDamping * Wy 
                          +  (-AircraftParameters.PitchDamping * Wy
                              - Math.Sin(Alpha) * Math.Sign(airspeedXY) * AircraftParameters.PitchStability 
                          ...
                          + (Thrust / (AircraftParameters.MaximumThrust + 4*totalLift + 1)) * gravityPlane.Z * AircraftParameters.PropCenter.X);
            Tz = (float)( - AircraftParameters.YawDamping * Wz 
                          + (-AircraftParameters.YawDamping * Wz +
                            ...
                            //+ AircraftParameters.WingCenter.Y * AircraftParameters.WingSpanFactor * (LiftLeft*Math.Sin(AlphaLeft) - LiftRight * Math.Sin(AlphaRight))
                            //+ AircraftParameters.WingCenter.Y * AircraftParameters.WingSpanFactor * (DragLeft*Math.Cos(AlphaLeft) - DragRight * Math.Cos(Alpha))
                           + AircraftParameters.RudderEfficiency * AircraftParameters.PropWashRudder * Throttle * Rudder
                           //+ (DragLeft*Math.Cos(AlphaLeft) - DragRight*Math.Cos(AlphaRight)) * AircraftParameters.WingSpanFactor * AircraftParameters.WingCenter.Y
                           - (Thrust/(AircraftParameters.MaximumThrust + 4*totalLift + 1)) * gravityPlane.Y  *AircraftParameters.PropCenter.X
                           );
            //Ty *= (float)(1 - 0.5f * Throttle);
            //Tz *= (float)(1 - 0.9f * Throttle);
        }
        ...
        private void CalculateTorquesSailboat(float elapsedTime, Vector3 wind)
        {
            ...
            Tz = (float)((-AircraftParameters.YawDamping * Wz -
                           Math.Sin(Beta) * Math.Sign(airspeedXZ) * AircraftParameters.YawStability +
                           AircraftParameters.RudderEfficiency * Vx * Rudder * Math.Sign(Vx)) * Math.Sqrt(Vx*Vx + Vz*Vz));
        }
        ...
        private void UpdateCollisions2(float elapsedTime)
        {
            ...
            Vector3 rotVelW = ToWorldCoords(new Vector3(Wx, Wy, Wz));
